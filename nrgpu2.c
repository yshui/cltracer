...
cl_kernel kernel = 0, motion_kernel = 0;
GLuint texture = 0;
cl_program progs[3];
int prgc,kc,nomotion=0;
cl_kernel kerns[3];
float imy[4*480*480];
float get_time(){
	struct timespec tp;
...
void startKernel(int w, int h) {
	const size_t global_ws[] = {SCRWIDTH, SCRHEIGHT};
	const size_t mgs[]={1,1};

	cl_int error;
	cl_event kr, mo;
	error = clEnqueueAcquireGLObjects(queue, 1, &tex_cl, 0, NULL, NULL);assert(!error);
	...
	if(!nomotion){
		...
		error = clEnqueueNDRangeKernel(queue, kernel, 2, NULL, global_ws, NULL, 1, &mo, &kr); assert(!error);
	}else{
		error = clEnqueueNDRangeKernel(queue, kernel, 2, NULL, global_ws, NULL, 0, NULL, &kr); assert(!error);
	}


	error = clEnqueueReleaseGLObjects(queue, 1, &tex_cl, 0, NULL, NULL); assert(!error);
	error = clFinish(queue); assert(!error);
	//cl_ulong skr, ekr, sac, eac, srl, erl;
	//clGetEventProfilingInfo(kr, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &skr, NULL);
	//clGetEventProfilingInfo(kr, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &ekr, NULL);
	//fprintf(stderr, "Kernel %f\n", (ekr-skr)/1e9);
...
void clInit() {
	...
	for(i=0; i<platform; i++){
		...
		error = clGetDeviceIDs(platform[i], CL_DEVICE_TYPE_GPU, 1, &device, NULL);
		if(!error)break;
	}
	assert(i<platform);
...
cl_program loadAndBuildProgram(const char *filename){
	...
	cl_int berr = clBuildProgram(prog, 1, &device, " -cl-strict-aliasing -cl-unsafe-math-optimizations -cl-finite-math-only", NULL, NULL);
...
void appInit(int w, int h) {
	float *tb = malloc(sizeof(float)*SCRWIDTH*SCRHEIGHT*4);
	int i;
	for(i=0;i<4*SCRWIDTH*SCRHEIGHT;i++)
		tb[i] = (float)rand()/(float)RAND_MAX;
	glGenTextures(1, &texture);
	glBindTexture(GL_TEXTURE_2D, texture);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, SCRWIDTH, SCRHEIGHT, 0, GL_RGBA, GL_FLOAT, tb);
...
//	assert(!error);
	cl_program prog = loadAndBuildProgram("nrn.cl");
