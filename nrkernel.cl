...
inline float get_lightning(float4 io, float4 np, float4 p, int lidx,
	...
	for(i=lidx;i<sc;i++){
		...
		if(idx==-1||tmp*tmp>tmpl-eps||idx==i){
			...
			if(t1*t3<1e-10){
				//printf("%f\n", sqrt(dot(np,np)));
				light+=t1*0.8*(1+max(0.0f, dot(rtp, -p)));
			}
		}
	}
...
inline float4 ray_trace(float4 p, float4 o, int pc, int sc, int lc,
	__constant float4 *po, __constant float4 *pn,
	...
	while(1){
		...
		float light = get_lightning(io, np, trp, tro, pc, sc, po, pn, so, sr, type);
		...
		get_reflection(np, trp, tro, io, so, pn, &tro, &trp);
		factor *= ore[idx2];
		depth++;
	}
	return color;
}
__kernel void work(uint pc, uint sc,
	__constant float4 *po, __constant float4 *pn, 
	__constant float4 *so, __constant float *sr,
	...
	o.y=-1;
	float4 clr=0;
	clr = ray_trace(p, o, pc, sc, po, pn, so, sr, ore, oc);
...
inline void photon_trace(float4 p, float4 o, int pc, int sc,
	__constant float4 *po, __constant float4 *pn,
	__constant float4 *so, __constant float *sr,
	__constant float *ore, __constant float4 *oc,
	...
	float4 trc = 1;
	int depth = 0;
	int randi = get_global_id(0)*get_global_id(1);
	while(1){
		if(depth>RTDEPTH)break;
	...
	*photon = (float8)(tro.x, tro.y, tro.z, 0, trc.x, trc.y, trc.z, 0);
}
...
__kernel void photon_generate(int pc, int sc, int lidx,
			__global float4 *po, __global float4 *pn,
			__global float4 *so, __global float4 *sr,
			__global float4 *oc, __global float4 *ore
			__global float4 *photon, __global int *gridc,
			__global float4 *pho, __global float4 *php
			__global float *rand){
	int phm=get_global_id(1);
	photon_trace(php[phm], pho[phm], pc, sc, po, pn, so, sr, ore, oc, photon+phm, rand);
	int id=grid_index(photon[phm].x, photon[phm].y, photon[phm].z);
	atomic_add(gridc+id, 1);
}
__kernel void photon_place(__global float4 *photon, __global int *gridc, __global int *grid, __local int *tmpg){
	int i, tmp=gridc[0];
	gridc[0]=0;
